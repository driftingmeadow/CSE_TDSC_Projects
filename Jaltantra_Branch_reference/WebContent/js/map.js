			...
			function(results, status) {
				...
				// Retrieve the first result
				...
				} else if(status==google.maps.ElevationStatus.OVER_QUERY_LIMIT)
					{
						...
						setTimeout(updateMarkerElevation, delay,marker, popup);
...
function updateElevation(polyline)
{
	elevationService.getElevationAlongPath({
		...
		if(results===null)
			alert(status);
		console.log('update pipe elevation success, the final delay was: '+delay+'ms.'); 
		delay = 100;
...
function getNodesJSON()
{
	...
	for(i=0;i<nodes.length;i++)
	{
		...
		jsonString = jsonString + delimiter 
						...
						+ '","elevation":"' + (node.elevation || 0)  // Added elevation - sends elevation data from map to server
						+ '","isesr":"' + node.isesr
						+ '"}';
		delimiter = ',';				
	}
...
function loadNodesFromJSON(jsonString)
{
	...
	for(i=0;i<jsonNodes.length;i++)
	{
		...
		//sending with delay
		setTimeout(addMarker, delay*i, position, false, jsonNode.nodeid, jsonNode.nodename);
	}
}
...
function loadPipesFromJSON(jsonString)
{
	...
	for(i=0;i<jsonPipes.length;i++)
	{
		...
		setTimeout(addPath, delay*i, path, origin, destination); //updated on 03-03-21
	}
}
